...
class Theme {
    ...
    initToc() {
        ...
        if (document.getElementById('toc-static').getAttribute('kept') || this.util.isTocStatic()) {
            ...
            const headerMenuHeight = document.getElementById('header-desktop').offsetHeight;
            const headerHeight = document.querySelectorAll('#header-desktop .header-bg-wrapper')[0].offsetHeight;
            const TOP_SPACING = (headerIsFixed ? headerMenuHeight : 0);
            // const minTocTop = $toc.offsetTop;
            // const minScrollTop = minTocTop - TOP_SPACING + (headerIsFixed ? 0 : headerHeight);
            // sidebar stuff 依樣畫葫蘆 lol :3
            $sidebar.style.left = `${rect.left + rect.width + 20}px`;
            $sidebar.style.maxWidth = `${$page.getBoundingClientRect().left - 70}px`;
            $sidebar.style.visibility = 'visible';
            const minSidebarTop = 20 + headerHeight;
            const minScrollTop = minSidebarTop - TOP_SPACING;
            // on scroll shit
            this._tocOnScroll = this._tocOnScroll || (() => {
                const footerTop = document.getElementById('post-footer').offsetTop;
                // const maxTocTop = footerTop - $toc.getBoundingClientRect().height;
                const maxTocTop = footerTop - $sidebar.getBoundingClientRect().height + headerHeight;
                ...
                const INDEX_SPACING = 20 + (headerIsFixed ? headerMenuHeight : 0);
    ...
    onScroll() {
        ...
        const $headerDesktop = document.querySelectorAll('#header-desktop .header-wrapper')[0];
        ...
        window.addEventListener('scroll', () => {
            ...
            if (this.newScrollTop > 0) {
                if (!isMobile || scroll < - ACCURACY) {
                    $headerDesktop.setAttribute('is-sticky', 'true');
                }
            } else {
                if (!isMobile) {
                    $headerDesktop.setAttribute('is-sticky', 'false');
                }
            }
    ...
    onClickSidebarIntroTabs() {
        const sidebarIntro = document.getElementById('sidebar-intro');
        if (sidebarIntro != null) {
            const sidebarIntroTabs = sidebarIntro.getElementsByClassName('sidebar-intro-tab');
            const sidebarIntroContent = sidebarIntro.getElementsByClassName('sidebar-intro-content');
            const tabSelector = ".sidebar-intro-content-container .sidebar-intro-content";
            sidebarIntro.querySelectorAll(`${tabSelector}[tab-index="0"]`)[0].classList.remove('hide-content');
            sidebarIntro.querySelectorAll(`.sidebar-intro-tab[tab-index="0"]`)[0].classList.add('sidebar-intro-tab-active');
            for (let i = 0; i < sidebarIntroTabs.length; i++) {
                const sidebarIntroTab = sidebarIntroTabs[i];
                sidebarIntroTab.addEventListener('click', () => {
                    for (let j = 0; j < sidebarIntroContent.length; j++) {
                        sidebarIntroTabs[j].classList.remove('sidebar-intro-tab-active');
                        sidebarIntroContent[j].classList.add('hide-content');
                    }
                    const sidebarIntroSingleContent = sidebarIntro.querySelectorAll(`${tabSelector}[tab-index="${i}"]`)[0];
                    sidebarIntro.querySelectorAll(`.sidebar-intro-tab[tab-index="${i}"]`)[0].classList.add('sidebar-intro-tab-active');
                    sidebarIntroSingleContent.classList.remove('hide-content');
                }, false);
            }
        }
    }
